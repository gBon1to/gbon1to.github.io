...
gSAST requires "grep" to work and uses its patterns files to find vulnerable code. At the moment, the vulnerabilities identified by the pattern files are mainly focused on the OSWE certification, but in the future I will add more.
...
``
gSAST.sh --> patterns/"source code language"/"vulnerability_type".rule --> Vulnerable Code
``
...
## Running gSAST
Starting of by running gSAST it possible to see many interesting findings such as:
...
I've used the following gSAST flags to include only PHP files:
...
After understanding the web application purpose and looking at gSAST output, my main focus was to get administrative access, and for that I've focused on Cross-Site Scripting vulnerabilities, since I had "write privileges" to the comments section.
...
To confirm this finding I accessed the "post.php" class file and saw a call for a "get_comments()" function, so I've searched for its usage in the code to fully understand it.

![](/assets/img/content/0d25d544740940f69addac7579a96e0e.png)
*Analysis of the "render_with_comments()" function*

The "get_comments()" function simply requests the "comments" table for its data, given a post identifier, and returns it back to the user. The only validation in place is in the "post_id" parameter that check if the input is an integer. No other validation is in place to prevent Cross-Site Scripting.

![](/assets/img/content/a08fd1817a1c4280b955cd51eb669977.png)
*Analysis of the "get_comments()" function*

Since my goal was to get administrative access to the application, I've injected a simple script tag that would load an external Javascript file from my server with the purpose stealing an administrator cookie.
...
With the administrator cookie on hands I just had to use it to access the administration panel, as seen in the picture below.

![](/assets/img/content/e16fc9b40a1e41e5b8040452f4365b7d.png)
*Administration panel access*

## SQL Injection
With administrative access to the web application, focus now was to get code execution on the server. Going back to the initial gSAST results, there were still a few blocks of code with possible SQL Injection vulnerabilities to analyze.
...
But once again to confirm this finding, I to access the file that contained the vulnerable code and further analyze it.

![](/assets/img/content/68190ad5e2a4436c8469ac170bf50ac1.png)
*Analysis of the "get_comments()" function*

As seen by the picture above the "find()" function was responsible for finding posts by their "id". Since in this function there was no input sanitization the only thing left to confirm the vulnerability was to analyze its calls. For this purposed I've used "grep" once again.
...
From the previous analysis it was possible to conclude, that the injected query would need to have a UNION SELECT statement to concat the information from the "posts" table with any other table.
Since a UNION query needs to have the exact same number of collums as the initial SELECT query, I've used the previously gathered database credentials to analyze the "posts" table structure and find the exact number of columns to include in the PoC query.
...
Success! I was now able to fully compromise the database data by injection malicious SQL queries.

### From SQL Injection to Remote Code Execution
To match our goal and complete this challenge, I had to inject an SQL query that would allow me to get code execution on the server.

For this purpose I've used a well known method that allowed arbitrary file write by using the SQL "INTO OUTFILE" statement.
...
After sending the request, I've noticed the "cmd.php" file was created and contained the malicious PHP code, as seen in the picture below.
...
- [https://github.com/gBon1to/gSAST](https://github.com/gBon1to/gSAST)